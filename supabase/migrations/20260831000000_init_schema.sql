...
-- id는 uuid 타입이 아니라 text다 — 애플리케이션(src/lib/repositories/mock-utils.ts의 createId())이
-- "song-<uuid>"처럼 사람이 읽을 수 있는 접두어를 붙인 문자열을 직접 발급해 저장 시점에 넘기므로,
-- DB가 default gen_random_uuid()로 별도 id를 생성하지 않는다.
...
create table if not exists sections (
  ...
  repeat_target_section_id text references sections (id) on delete set null,
