...
// 메시지를 보여줄 수 있게 한다. fetch()/res.json() 자체는 항상 errors.ts의 헬퍼를 거친다.
import type {
  ApiErrorBody,
...
export async function fetchSetlists(): Promise<ListSetlistsResponse> {
  const res = await fetchOrThrowNetworkError("/api/setlists");
  if (!res.ok) {
    throw new Error(
      await parseErrorMessage(res, `찬양콘티 목록 조회에 실패했습니다 (status ${res.status}).`),
    );
  }
  return parseJsonOrThrowNetworkError<ListSetlistsResponse>(res);
}

export async function fetchSetlist(setlistId: string): Promise<GetSetlistResponse | null> {
  const res = await fetchOrThrowNetworkError(`/api/setlists/${setlistId}`);
  if (res.status === 404) return null;
  if (!res.ok) return throwForStatus(res, `찬양콘티 조회에 실패했습니다 (status ${res.status}).`);
  return parseJsonOrThrowNetworkError<GetSetlistResponse>(res);
}
...
export async function createSetlist(request: CreateSetlistRequest): Promise<CreateSetlistResponse> {
  ...
  return parseJsonOrThrowNetworkError<CreateSetlistResponse>(res);
}
...
export async function renameSetlist(
  ...
  return parseJsonOrThrowNetworkError<UpdateSetlistResponse>(res);
}
...
export async function updateSetlistItems(
  ...
  const res = await fetchOrThrowNetworkError(`/api/setlists/${setlistId}/items`, {
    method: "PATCH",
    headers: { "content-type": "application/json" },
    body: JSON.stringify(request),
  });
...
export async function deleteSetlist(setlistId: string): Promise<void> {
  const res = await fetchOrThrowNetworkError(`/api/setlists/${setlistId}`, { method: "DELETE" });
