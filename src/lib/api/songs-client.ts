...
export async function fetchSongs(params?: {
  ...
  const res = await fetchOrThrowNetworkError(`/api/songs${qs ? `?${qs}` : ""}`);
  if (!res.ok) {
    throw new Error(
      await parseErrorMessage(res, `곡 목록 조회에 실패했습니다 (status ${res.status}).`),
    );
  }
  return parseJsonOrThrowNetworkError<ListSongsResponse>(res);
}

export async function deleteSong(songId: string): Promise<void> {
  const res = await fetchOrThrowNetworkError(`/api/songs/${songId}`, { method: "DELETE" });
  if (!res.ok) {
